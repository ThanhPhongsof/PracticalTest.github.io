...
var pnlContainer2 = '';
var pnlDragUpload2 = '';
var fileUploadControl2 = '';
var isFullUpload2 = '2';
//$(document).ready(function () {
//    ReloadDragUpload();
//});

$.fn.bindDragUpload2 = function () {
    $(this).html5Uploader({
        name: "upfilethum",
        postUrl: "/Tool/Upload.ashx?isfll=" + isFullUpload,
        onServerLoadStart: function (e, file) { // dau tien
            $.blockUI({ message: '<h3><i class="icon-spinner icon-spin blue bigger-125" style="font-size:30px;"></i> Vui lòng chờ chút...</h3>', css: { color: '#fff' } });
        },
        ...
        onSuccess: function (e, file) {
            ...
            if (data != '-1') {
                ...
                var fileGuidList = $('#hdGuidName2').val();
                // add file NAME moi vao mang
                var fileNameList = $('#hdFileName2').val();
                // image text html
                var imgHTML = '';
                for (i = 0; i < data.length; i++) {
                    ...
                    imgHTML += '<img src="' + filePath + '" alt="' + fileName + '" width="100%" style="margin-top: 4px;" />';
                    fileGuidList = fileGuid;
                    fileNameList = fileName;
                }
                $('#' + pnlContainer2).html(imgHTML);
                
                $('#hdGuidName2').val(fileGuidList);
                $('#hdFileName2').val(fileNameList);
                $('#' + pnlContainer2).removeAttr('style');
            }
            if (data == '-1') {
...
function ReloadDragUpload2() {
    $('#' + pnlDragUpload2).bindDragUpload2();
    $('#' + fileUploadControl2).bindDragUpload2();
}
...
$(document).on('click', 'span.cmd-del-file', function () {
    if (window.confirm('Bạn chắc chắn muốn xóa file này?') == false) {
        ...
        if (boolRlt) {
            ...
            RemoveFileFromList(fileGUID, 'hdGuidName2');
            // xoa file NAME khoi ds
            RemoveFileFromList(fileName, 'hdFileName2');
